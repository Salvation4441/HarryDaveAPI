...
def create(request: schemas.Category, db: Session = Depends(conn),get_current_user: schemas.Staff = Depends(oauth2.get_current_user)):
...
def get_category_id(id, db: Session = Depends(conn),get_current_user: schemas.Staff = Depends(oauth2.get_current_user)):
...
def update_category(id, request: schemas.Category, db: Session = Depends(conn),get_current_user: schemas.Staff = Depends(oauth2.get_current_user)):
